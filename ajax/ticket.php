...
switch ($_GET["op"]){
		...
		$rspta=$venta->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			...
 			$data[]=array(
				 ...
				 '<a target="_blank" href="'.$url_ticket.$reg->idventa.'"> <button class="btn btn-info"><i class="fa fa-print"></i></button></a>'.
	...
	case 'listarArticulosVenta':
		...
		foreach ($rspta as $item) {
			...
			$params = 
			$data[]=array(
				"0"=>'<button class="btn btn-warning" onclick="agregarDetalle('.@$item['idarticulo'].',\''.@$item['nombre'].'\',\''.$unidadm.'\',\''.@$item['precio_venta'].'\',\''.@$item['afectacion'].'\',\''.@$item['codigo'].'\')"><span class="fa fa-plus"></span></button>',
				"1"=>@$item['nombre'],
				"2"=>$unidadm,
				"3"=>@$item['categoria'],
				"4"=>@$item['codigo'],
